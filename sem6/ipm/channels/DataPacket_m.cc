...
DataPacket::DataPacket(const char *name, int kind) : ::queueing::Job(name,kind)
{
    this->size_var = -1;
}

DataPacket::DataPacket(const DataPacket& other) : ::queueing::Job(other)
{
    copy(other);
}
...
DataPacketDescriptor::DataPacketDescriptor() : cClassDescriptor("DataPacket", "queueing::Job")
{
}
