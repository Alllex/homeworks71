...
next c a p b = p (c a) b

left_assoc c p op = p ||> (\x -> op ||> (\o -> p ||> (\y -> val $ next c x o))

bin_op l r op = l ||> (\x -> op ||> (\o -> r ||> (\y -> val $ x `o` y))) ||| l

multi c = left_assoc c primary op
    where op = sym1 '*' MUL ||| sym1 '/' DIV

addi c = left_assoc c (multi id) op
    where op = sym1 '+' ADD ||| sym1 '-' SUB              

reli = bin_op (addi id) (addi id) op
    where op = sym2 '>' '=' RNLT |||
               sym2 '<' '=' RNGT |||
               sym2 '=' '=' REQ  |||
               sym2 '!' '=' RNE  |||
